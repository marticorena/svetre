...
const resolvers = {
    Query: {
        ...
        horse: async (_, { id }) => prisma.horse.findUnique({ where: { id } }),
        departments: async () => prisma.department.findMany(),
    },
    Mutation: {
        addHorse: async (_, args) => {
    ...
    Horse: {
        department: async (parent) => parent.departmentId ? prisma.department.findUnique({ where: { id: parent.departmentId } }) : null,
        histories: async (parent) => prisma.departmentHistory.findMany({ where: { horseId: parent.id }, orderBy: { startDate: 'desc' } }),
        sire: async (parent) => parent.sireId ? prisma.horse.findUnique({ where: { id: parent.sireId } }) : null,
        dam: async (parent) => parent.damId ? prisma.horse.findUnique({ where: { id: parent.damId } }) : null,
        sired: async (parent) => prisma.horse.findMany({ where: { sireId: parent.id } }),
        damed: async (parent) => prisma.horse.findMany({ where: { damId: parent.id } }),
        hasParents: (parent) => !!(parent.sireId || parent.damId),
        hasChildren: async (parent) => {
            const children = await prisma.horse.count({ where: { OR: [{ sireId: parent.id }, { damId: parent.id }] } });
            return children > 0;
        },
        parentCount: (parent) => (parent.sireId ? 1 : 0) + (parent.damId ? 1 : 0),
        childCount: async (parent) => prisma.horse.count({ where: { OR: [{ sireId: parent.id }, { damId: parent.id }] } }),
    },
    DepartmentHistory: {
        department: async (parent) => prisma.department.findUnique({ where: { id: parent.departmentId } })
    }
};
const app = express();
